...
# Тестовые изображения 000010.jpg - 000012.jpg
test_image_ids = ['000010.jpg', '000011.jpg', '000012.jpg']
...
def crop_face(img_name):
    ...
    x1, y1, width, height = bbox_row['x_1'], bbox_row['y_1'], bbox_row['width'], bbox_row['height']
...
def test_face_alignment():
    ...
    for idx, img_name in enumerate(test_image_ids):
        print(f"🔄 {img_name}")
        
        cropped_pil, original_img = crop_face(img_name)
        
        img_resized = cropped_pil.resize((112, 112), Image.BICUBIC)
        ...
        with torch.no_grad():
            ...
            keypoints = get_keypoints_from_heatmaps(pred_heatmaps)[0].numpy()
        
        orig_rgb_hw = np.array(img_resized)
        aligned_rgb = align_face_numpy(orig_rgb_hw, keypoints)
        ...
        axes[row, 1].imshow(aligned_rgb)
        axes[row, 1].set_title('ВЫРОВНЕННОЕ')
        axes[row, 1].axis('off')
    
    plt.suptitle('ОРИГИНАЛ → ВЫРОВНЕННОЕ\n(000010.jpg - 000012.jpg)', fontsize=18)
    plt.tight_layout()
    plt.show()
